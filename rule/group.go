...
const (
	...
	// DefaultDialBudget (seconds) is the wall clock retries may spend. Keep it
	// below the dial timeout of whatever client sits in front of glider:
	// overshooting replaces a fast 502 with a slow timeout, which is strictly
...
// rotation, one flaky upstream should not become a 502 downstream: the common
// failure is per-forwarder (an anytls SYNACK timeout, a TCP i/o timeout, an
// EOF), not a whole-group outage.
//
