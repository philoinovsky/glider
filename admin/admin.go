...
// the consumers are Torii (which already serves and parses Prometheus text) and
// Perennial (which already scrapes it). Reusing that format means neither side
// needs new plumbing, and a human can curl the same URL. The JSON /state
// endpoint carries the identical data for eyeballing and ad-hoc scripts.
//
